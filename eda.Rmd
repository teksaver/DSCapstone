...
ngramsOcc <- function(lines){
    ...
    for (line in lines){
        ...
        for(i in 1:length(words)){
            ...
            if(i>1){#do nothing for the first token
                twoGram <- paste(words[i],words[i-1],sep=" ")
                if(is.null(occurTwo[[words[i]]])){
                    occurTwo[[words[i]]] <- 1
                    #else increment the number of times the word is seen
                }else{
                    occurTwo[[words[i]]] <- occurTwo[[words[i]]] +1
                }
            }
    ...
    twoGramOccDf <- as.data.frame(unlist(as.list(occurTwo)))
    ...
    return(list(oneGram=wordOccDf,twoGram=twoGramDf))
...
blogsWords <- ngramsOcc(blogsSubset)$oneGram
newsWords <- ngramsOcc(newsSubset)$oneGram
twittWords <- ngramsOcc(twittSubset)$oneGram
blogsTwoGram <- ngramsOcc(blogsSubset)$twoGram
```
