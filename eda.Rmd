...
blogsSubset <- blogsLines[sample(1:length(blogsLines)/10, 1000, replace=FALSE)]
newsSubset <- blogsLines[sample(1:length(newsLines)/10, 1000, replace=FALSE)]
twittSubset <- blogsLines[sample(1:length(twittLines)/10, 1000, replace=FALSE)]
...
```{r ngrams analysis}
blogsWords <- blogsNGrams$oneGram
newsWords <- newsNGrams$oneGram
twittWords <- twittNGrams$oneGram
```

## 2-gram analysis

```{r 2gram analysis}
blogsTwoGram <- blogsNGrams$twoGram
```
